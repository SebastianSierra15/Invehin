...
<%
    ...
    int totalPrendas = (prendas != null) ? prendas.size() : 0;
%>
...
<html  lang="es">
    ...
    <body class="bg-invehin-background font-sans flex">
        <aside id="sidebar" class="fixed top-0 left-0 bg-invehin-accent h-screen w-64 flex flex-col justify-between border-r-2 border-black transition-all duration-300 p-4">
            <div id="logo-container" class="flex items-center justify-between mb-4 pb-2 bg-invehin-accent border-b-2 border-black w-full sticky top-0 z-10 transition-all duration-300">
                <a href="${pageContext.request.contextPath}/" class="flex items-center justify-center">
                    <img src="images/logo.webp" title="Invehin" alt="Invehin Logo" class="w-32" id="sidebar-logo" />
                </a>
        ...
        <main id="main-content" class="flex-1 p-8 ml-64 transition-all duration-300">
            <div class="text-center mb-6">
                <h1 class="text-invehin-primary font-bold text-3xl mb-2">Gestión de Prendas</h1>
            </div>

            <div class="flex justify-between items-center mb-4">
                <h2 class="text-invehin-primaryLight text-xl font-semibold">Categorías</h2>
                <button class="bg-invehin-primary text-white px-4 py-2 rounded shadow hover:bg-invehin-primaryLight">
                    <i class="fas fa-plus mr-1"></i> Agregar Prenda
                </button>
            </div>

            <div class="bg-invehin-accentLight p-4 rounded shadow border border-black">
                <div class="flex justify-between mb-2">
                    <div>
                        <label class="text-sm font-semibold mr-2">Mostrar</label>
                        <select class="px-2 py-1 border rounded">
                            <option>10</option>
                            <option>20</option>
                            <option>30</option>
                        </select>
                        <span class="ml-2">prendas</span>
                    </div>
                    <div>
                        <input type="text" placeholder="Buscar prenda" class="px-3 py-1 rounded border w-64"/>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full table-auto border-collapse">
                        <thead>
                            <tr class="bg-invehin-backgroundAlt">
                                ...
                                <th class="border px-4 py-2">Stock Mínimo</th>
                        ...
                        <tbody>
                            <% if (prendas != null && !prendas.isEmpty())
                                {
                                    ...
                                    {%>
                            <tr class="odd:bg-white even:bg-invehin-backgroundLighter">
                                <td class="border px-4 py-2"><%= p.subcategoriaPrenda.categoriaSubcategoria.nombreCategoria%></td>
                                <td class="border px-4 py-2"><%= p.subcategoriaPrenda.nombreSubcategoria%></td>
                                <td class="border px-4 py-2"><%= p.tallaPrenda.nombreTalla%></td>
                                <td class="border px-4 py-2"><%= p.colorPrenda.nombreColor%></td>
                                <td class="border px-4 py-2">$<%= p.subcategoriaPrenda.precioSubcategoria%></td>
                                <td class="border px-4 py-2"><%= p.stockPrenda%></td>
                                <td class="border px-4 py-2"><%= p.stockminimoPrenda%></td>
                                <td class="border px-4 py-2 text-center">
                                    <button class="text-blue-600 hover:text-blue-800 mr-2" title="Editar">
                            ...
                            { %>
                            <tr>
                                <td class="text-center px-4 py-4 text-gray-500 font-semibold" colspan="9">No hay prendas registradas</td>
                            </tr>
                            <% }%>
                        </tbody>
                    </table>
                ...
                <div class="flex justify-between items-center mt-4">
                    <span class="text-sm font-semibold">Mostrando 1 a 10 de <%= totalPrendas%> prendas</span>
                    <div class="flex gap-2">
                        <button class="px-3 py-1 border border-black rounded bg-white hover:bg-invehin-primaryLight">&lt;</button>
                        <button class="px-3 py-1 border border-black rounded bg-invehin-primary text-white">1</button>
                        <button class="px-3 py-1 border border-black rounded bg-white hover:bg-invehin-primaryLight">2</button>
                        <button class="px-3 py-1 border border-black rounded bg-white hover:bg-invehin-primaryLight">3</button>
                        <button class="px-3 py-1 border border-black rounded bg-white hover:bg-invehin-primaryLight">&gt;</button>
                    </div>
                </div>
            </div>
